...
echo IP:
tasklist | find /i "ngrok.exe" >Nul && curl -s localhost:4040/api/tunnels | jq -r .tunnels[0].public_url || echo "Can't get NGROK tunnel, pls update NGROK TOKEN in YML. Maybe your previous VM still running: https://dashboard.ngrok.com/status/tunnels " && exit 
